 ...
 * the IEthernetManager interface. It also creates a EthernetMonitor to listen
 * for Ethernet-related events.
 *
 * @hide
 */
public class EthernetService<syncronized> extends IEthernetManager.Stub {
    ...
    public synchronized void setMode(String mode) {
        final ContentResolver cr = mContext.getContentResolver();
        if (DevName != null) {
        	if(DevName[0].equals("eth0"))
        	{
        		Settings.Secure.putString(cr, Settings.Secure.ETHERNET_IFNAME, DevName[0]);
        	}
        	else if ((DevName.length > 1) && DevName[1].equals("eth0"))
        	{
        		Settings.Secure.putString(cr, Settings.Secure.ETHERNET_IFNAME, DevName[1]);
        	}
        	else
        	{
        		Settings.Secure.putString(cr, Settings.Secure.ETHERNET_IFNAME, DevName[0]);
